...
with Window(u"Текущая история мониторинга||Window"):
    with Region(u"||SplitButton->||SplitButton->||Custom"):
        left_click(u"Шкала X||Button")

with Window(u"Масштабирование графика параметра по оси X||Window"):
    left_click(u"||TitleBar")
    left_click(u"Ок||Button")

with Window(u"Текущая история мониторинга||Window"):
    with Region(u"||SplitButton->||SplitButton->||Custom->||Custom->||Custom->||Custom->||Custom"):
        left_click(u"Шкала Y||Button")

with Window(u"Масштабирование графика параметра по оси Y||Window"):
    left_click(u"||TitleBar")
    left_click(u"Отменить||Button")

with Window(u"Текущая история мониторинга||Window"):
    with Region(u"||SplitButton->||SplitButton->||Custom->||Custom->||Custom->||Custom->||Custom"):
        left_click(u"㏒||CheckBox")
